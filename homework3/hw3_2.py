...
def wilsons_theta_method(time, ag, m, k, c, u0, v0, dt, theta=1.4):
    n_steps = len(ag)
    ...
    for i in range(n_steps - 1):
        ...
        rhs = F_theta + \
            ...
            c * (3*u[i]/(theta*dt) + 2*v[i] + theta*dt*a_resp[i]/2)

        u_theta = rhs / k_eff

        a_resp[i+1] = (6/(theta**2*dt**2)) * (u_theta - u[i] - theta*dt*v[i]) - 2*a_resp[i]
        v[i+1] = v[i] + dt * (a_resp[i] + a_resp[i+1]) / 2
        u[i+1] = u[i] + dt * v[i] + (dt**2 / 6) * (a_resp[i] + a_resp[i+1] + a_resp[i+1])
