...
def system_matrices_single_tmd(tmd_idx, md_r_list, omega_r_list, zeta_r_list, normalized_modes):
    m1 = m2 = m3 = 8.46e7
    k1 = k2 = k3 = 71200000
    c1 = c2 = c3 = 1552226.752869
    ...
    # normalized_modes shape: (3, 3)，每一欄是一個模態
    M_modal = []
    for i in range(3):
        mode_vec = normalized_modes[:, i].reshape(-1, 1)  # (3, 1)
        M_i = float(mode_vec.T @ m @ mode_vec)  # 結果是 1x1 array，取 float
        M_modal.append(M_i)
    # M_modal = [M_1, M_2, M_3]
    m_main = M_modal[tmd_idx]
    print(f"M1={M_modal[0]}")
    ...
    # 4自由度系統（以下同你原本的）
    M = np.diag([M_modal[0], M_modal[1], M_modal[2], md])
    K = np.zeros((4, 4))
    C = np.zeros((4, 4))
    K[:3, :3] = np.array([
        [k1 + k2, -k2,     0],
...
def main():
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        for tmd_idx, floor_name in enumerate(["1F", "2F", "3F"]):
            tmd_num = [7, 8, 9][tmd_idx]
            M_tmd, K_tmd, C_tmd = system_matrices_single_tmd(tmd_idx, md_r_list, omega_r_list, zeta_r_list, modal_shapes)
