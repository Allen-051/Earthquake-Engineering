...
def displacement_loop(A_1, B_1, dt, u0, v0, a0_1, F, time_offset):
    ...
    return u_a, v_a, a_a, df

def matrix_2(m, u_a, v_a, k, zeta, dt, F):
    """計算第二階段參數與初始加速度"""
    delta = 0.5
    alpha = 0.25
...
def displacement_loop_2(A_2, B_2, dt, u_a, v_a, a_a, F, time_offset):
    ...
    for j in range(steps):
        ...
        if not found_b and x_j[1, 0] > 0 and x_j1[1, 0] <= 0:
            ...
            found_b = True
            cut_idx = j + 1
            print(f"***b點***速度為0時，u={u_b:.4f}, v={v_b:.4f}, a={a_b:.4f}(將做為第三階段初始值)")
            break  # 只保留到v變號為止
    ...
    print("x_hist (第二階段):")
    print(df.to_string(index=False))
    return u_b, v_b, a_b, df

def matrix_3(m, u_b, v_b, k, zeta, dt, F):
    """計算第三階段參數與初始加速度"""
    k = -k
    delta = 0.5
    alpha = 0.25
...
def displacement_loop_3(A_3, B_3, dt, u_b, v_b, a_b, F, time_offset):
    """第三階段，從b點開始完整計算"""
    steps = len(F) - 1
    x_hist = []
    ...
    for j in range(steps):
        ...
        if not found_c and (x_j[0, 0] > - u_c_control)>=0 and (x_j1[0, 0]-u_c_control) <= 0:
            ...
            found_c = True
            cut_idx = j + 1
            print(f"***c點***位移U_max-2Xy時，u={u_c:.4f}, v={v_c:.4f}, a={a_c:.4f}(將做第四階段初始值)")
            break 
    ...
    print("x_hist (第三階段):")
    print(df.to_string(index=False))
    return u_c, v_c, a_c, df

def matrix_4(m, u_c, v_c, k, zeta, dt, F):
    """計算第四階段參數與初始加速度"""
    # k = -k  
    delta = 0.5
    alpha = 0.25
...
def displacement_loop_4(A_4, B_4, dt, u_c, v_c, a_c, F, time_offset):
    ...
    for j in range(steps):
        ...
        if not found_d and x_j[1, 0] < 0 and x_j1[1, 0] >= 0:
            ...
            found_d = True
            cut_idx = j + 1
            print(f"***d點***速度為0時，u={u_d:.4f}, v={v_d:.4f}, a={a_d:.4f}(將做為第五階段初始值)")
    ...
    print("x_hist (第四階段):")
    print(df.to_string(index=False))
    return u_d, v_d, a_d, df

def matrix_5(m, u_d, v_d, k, zeta, dt, F):
    """計算第五階段參數與初始加速度""" 
    delta = 0.5
    alpha = 0.25
...
def displacement_loop_5(A_5, B_5, dt, u_d, v_d, a_d, F, time_offset):
    """第四階段，從c點開始完整計算或根據控制條件截斷"""
    steps = len(F) - 1
    x_hist = []
    ...
    for j in range(steps):
        ...
        if not found_e and (x_j[0, 0] - u_e_control) >=0 and (x_j1[0, 0] - u_e_control) <=0:
            ...
            found_e = True
            cut_idx = j + 1
            print(f"***e點***位移為-U_max+2Xy時，u={u_e:.4f}, v={v_e:.4f}, a={a_e:.4f}(將做為第六階段初始值)")
    ...
    x_time = np.vstack([time, x_hist])
    df = pd.DataFrame(x_time.T, columns=['time', 'u', 'v', 'a'])
    return u_e, v_e, a_e, df

def matrix_6(m, u_e, v_e, k, zeta, dt, F0):
    """計算第六階段參數與初始加速度"""
    delta = 0.5
    alpha = 0.25
...
def displacement_loop_6(A_6, B_6, dt, u_e, v_e, a_e, F, time_offset):
    """第六階段，從e點開始完整計算或根據控制條件截斷"""
    steps = len(F) - 1
    x_hist = []
    ...
    for j in range(steps):
        ...
        if not found_f and x_j[0, 0] <= 1 and x_j1[0, 0] >= 1:
            u_f = x_j1[0, 0]
            v_f = x_j1[1, 0]
            a_f = x_j1[2, 0]
            found_f = True
            cut_idx = j + 1
            print(f"***f點***位移回到Xy時，u={u_f:.4f}, v={v_f:.4f}, a={a_f:.4f}(分析中止)")
...
def plot_figures(df_all, k, save_path, filename):
    ...
    #plt.grid(True)
    #plt.savefig(os.path.join(save_path, f"{filename}_fig4_time_Fs.png"))
    #plt.close()
    #plt.show()
    ...
    #plt.grid(True)
    #plt.savefig(os.path.join(save_path, f"{filename}_fig5_u_Fs.png"))
    #plt.close()
    #plt.show()
...
def main():
    ...
    # 第一階段
    u_a, v_a, a_a, df1 = displacement_loop(A_1, B_1, dt, u0, v0, a0_1, F, time_offset=0.0)
    t1_end = df1['time'].iloc[-1]

    # 第二階段
    A_2, B_2, a0_2 = matrix_2(m, u_a, v_a, k2, zeta, dt, F0)
    u_b, v_b, a_b, df2 = displacement_loop_2(A_2, B_2, dt, u_a, v_a, a_a, F, time_offset=t1_end)
    t2_end = df2['time'].iloc[-1]

    # 第三階段
    A_3, B_3, a0_3 = matrix_3(m, u_b, v_b, k1, zeta, dt, F0)
    # 這裡要讓 displacement_loop_3 回傳 u_c, v_c, a_c, df3
    u_c, v_c, a_c, df3 = displacement_loop_3(A_3, B_3, dt, u_b, v_b, a_b, F, time_offset=t2_end)
    t3_end = df3['time'].iloc[-1]

    # 第四階段
    A_4, B_4, a0_4 = matrix_4(m, u_c, v_c, k2, zeta, dt, F0)
    u_d, v_d, a_d, df4 = displacement_loop_4(A_4, B_4, dt, u_c, v_c, a_c, F, time_offset=t3_end)
    t4_end = df4['time'].iloc[-1]

    # 第五階段
    A_5, B_5, a0_5 = matrix_5(m, u_d, v_d, k1, zeta, dt, F0)
    u_e, v_e, a_e, df5 = displacement_loop_5(A_5, B_5, dt, u_d, v_d, a_d, F, time_offset=t4_end)
    t5_end = df5['time'].iloc[-1]
    # 第六階段
    A_6, B_6, a0_6 = matrix_6(m, u_e, v_e, k2, zeta, dt, F0)
    df6 = displacement_loop_6(A_6, B_6, dt, u_e, v_e, a_e, F, time_offset=t5_end)
    ...
    # 輸出成一個 csv
    df_all.to_csv(os.path.join(save_path, f"{filename}_all.csv"), index=False)
